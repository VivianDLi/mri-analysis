...
def plot_cluster_map(df, normalize: bool = True):
    ...
    plt.savefig(
        f"{RESULTS_FOLDER}/clusters/clustermap-subset-{subset}-norm-{normalize}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.png"
    )
    plt.close()
...
def plot_pca_covariance(
    ...
    plt.savefig(
        f"{RESULTS_FOLDER}/pcas/covariance-add-{additional_columns}-group-{group_name}-norm-{normalize}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.png"
    )
    plt.close()
...
def plot_bgp_latents(df, n_components: int = 5):
    ...
    fig.savefig(
        f"{RESULTS_FOLDER}/gps/blvm-latents-feature-ncomp-{n_components}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.png"
    )
    plt.close()
